...
The dispersal kernel can be used in conjunction with transect data (area of transect, total number of seedlings per transect) to estimate the total number of seedlings in a 100m radius around each parent tree (assuming that the number of seedlings recruiting beyond that radius is very small).
...
The scaling of fruit production with tree size (from **(2) Fruiting volume model**) is:
$$\log(fruits) = \beta_{if} + \beta_{iD} DBH$$
And the scaling of recruitment with tree size can be represented similarly
$$\log(recruits) = \beta_{ir} + \beta_{iD} DBH$$
If we assume that $\beta_{iD}$ in both equations is equal, the only unknown parameter left in Eqn 2 is $\beta_{ir}$. We can solve for this:
$$\beta_{iD} DBH=\log(fruits)-\beta_{if}=\log(recruits)-\beta_{ir}$$
Thus,
$$\beta_{ir} DBH=\log(recruits)-\log(fruits)+\beta_{if}$$
